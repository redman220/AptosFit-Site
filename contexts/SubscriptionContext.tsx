...
interface SubscriptionContextType {
  ...
  /** Whether the user has VIP (Friends & Family) access — grants free Pro */
  isVip: boolean;
...
export function SubscriptionProvider({ children }: SubscriptionProviderProps) {
  ...
  const isSubscribed = rcSubscribed || isVip;
        ...
        customerInfoListener = Purchases.addCustomerInfoUpdateListener(
          (customerInfo) => {
            ...
            if (hasEntitlement || !__DEV__) {
              setRcSubscribed(hasEntitlement);
            }
          }
        );
  ...
  // Check VIP (Friends & Family) status whenever the authenticated user changes
  useEffect(() => {
    ...
    const checkVip = async () => {
      try {
        console.log("[VIP] Checking VIP status for user:", user.id);
        const data = await apiGet<{ isVip: boolean }>("/api/vip/check");
        console.log("[VIP] VIP status:", data.isVip);
        setIsVip(data.isVip);
      } catch (error) {
        // Non-fatal — VIP check failure should not block the app
        console.warn("[VIP] Failed to check VIP status:", error);
        setIsVip(false);
      }
    };
